...
- [x] **PUSH-5 Isi secret** — selesai (2026-07-18, atas izin user dikerjakan asisten): VAPID keypair BARU digenerate, disimpan di tabel `private.app_secrets` (schema private, tanpa grant anon/authenticated; dibaca Edge Function via RPC `get_app_secrets()` khusus service_role). Edge Function `send-push` v2 di-deploy membaca secret dari DB. Public key baru dipasang di env Vercel + .env.local.
- [x] **PUSH-6 Jadwalkan pg_cron → send-push** — job `send_push_every_5min` aktif (*/5 menit), cron secret diambil dari `private.app_secrets` saat runtime (tidak hardcoded di perintah cron). Diverifikasi: tanpa header 401, dengan header function berjalan. Catatan: subscription lama (VAPID lama) dibersihkan — user perlu Aktifkan ulang push di Settings.
